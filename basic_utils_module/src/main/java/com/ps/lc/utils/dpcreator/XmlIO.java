    ...
    public static void createDestinationDimens(boolean isFontMatch, ArrayList<DimenItem> list, double multiple, String outPutFile) {
        try {
            ...
            if (targetFile.exists()) {
                try {
                    System.out.println(outPutFile+"[没有被清除干净，接下来在进行清除]");
                    targetFile.delete();
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
            String decimalFormat = initDotNumber();
//            String decimalFormat = "0.00";
            ...
            for (int i = 0; i < size; i++) {
                ...
                //乘以系数，加上后缀
                String targetValue = Tools.countValue(isFontMatch, dimenBean.value, multiple, decimalFormat);
                attributes.clear();
    ...
    private static String initDotNumber() {
        int number = Constant.DOT_NUMBER;
        StringBuffer result = new StringBuffer(number > 0 ? "0." : "0");
